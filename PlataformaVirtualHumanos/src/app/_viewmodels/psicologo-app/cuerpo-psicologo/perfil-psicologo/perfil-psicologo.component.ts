...
@Component({
  ...
  styleUrls: ['./perfil-psicologo.component.scss']
})
export class PerfilPsicologoComponent implements OnInit , AfterViewInit{
  ...
  ngAfterViewInit(): void {
    ...
    console.log(usuario);
    this.psicologoService.leerPsicologo(usuario.id)
    .pipe(first())
    .subscribe((data:any) => {
      ...
      this.form.controls['fechaDeNacimiento'].setValue(this.convertirFechaYQuitarHoras(this.usuario.fecha_de_nacimiento));
  ...
  onSubmit(){
    ...
    this.psicologoService.modificarPsicologo(paciente)
    .pipe(first())
    .subscribe(() => {
